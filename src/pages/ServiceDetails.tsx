...
const ServiceDetails = () => {
  ...
  if (service.slug === "operationperformancemanagement" && service.fullContent) {
    const scopedHtml = getScopedBlogHtml(service.fullContent);
    return (
      <div className="min-h-screen bg-background">
        <div className="container mx-auto px-4 lg:px-8 py-12">
          <div className="grid gap-10 lg:grid-cols-[1.25fr_0.25fr] items-start">
            <div className="space-y-8">
              <div className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-6 py-3 text-sm font-medium text-foreground hover:border-secondary/60 transition">
                <Link to="/services" className="inline-flex items-center gap-2">
                  <ArrowLeft size={16} /> Back to Services
                </Link>
              </div>

              <div className="rounded-[2rem] border border-border bg-card p-8 shadow-2xl shadow-secondary/10">
                <div className="prose prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: scopedHtml }} />
              </div>
            </div>
